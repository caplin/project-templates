# Caplin Charting Adapter Template

This template project provides a starting point for writing charting integration adapters based on Caplin's Java [DataSource API](http://www.caplin.com/developer/component/datasource).
...
Follow the instructions below to create a new adapter project based on the Chart Adapter Template.
...
1. Copy the template directory `charting-template` and rename it to the name of your new project (for example, MyChartingAdapter):

    ```bash
    cp -r ./charting-template ~/src/MyChartingAdapter
    ```

1. Edit the file `~/src/MyChartingAdapter/settings.gradle`, and change the value of the `rootProject.name` variable to the name of your adapter project (MyChartingAdapter). When you later export your project as an [adapter blade](http://www.caplin.com/developer/component/deployment-framework/features-and-concepts/cdf-blade-types#Adapter-blade), the project name will be used as the name for the blade.

1. Edit the file `~/src/MyChartingAdapter/blade/blade_config/bootstrap.conf`. Set the value of the configuration variable `ROUTE_VIA_TRANSFORMER` to `TRUE` (default) to configure Liberator to route requests to the adapter via Transformer or `FALSE` to configure Liberator to route requests directly to the adapter.
...
1. Supply your project's dependencies manually, or configure Gradle to download them automatically from the Caplin software repository (coming soon).
    ...
    * **Manual download**: copy the latest versions of the following Caplin software libraries to the `~/src/MyChartingAdapter/lib` directory:

        * <code>datasource-java-<em>version</em>-jar-with-dependencies.jar</code>
        * <code>chartingAPI-<em>version</em>.jar</code>

    * **Caplin software repository**: (coming soon.) Edit the file `~/src/MyChartingAdapter/build.gradle`, and supply the credentials to your Caplin account in the `repositories` section.
...
1. In the **Location** field, select your adapter's project directory: `~/src/MyChartingAdapter`
...
1. Select the project's Gradle build file: `~/src/MyChartingAdapter/build.gradle`
